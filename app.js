...
function renderLeaderboard(){
 ...
 else{const rows=calculateLeaderboard(def),unit=def.type==='scratch'?'strokes':def.type==='putts'?'putts':'pts';body=rows.map((r,i)=>{const waiting=(def.type==='putts'||def.type==='best3')&&!r.thru;return`<div class="leaderRow ${i===0&&r.thru?'winner':''} ${waiting?'waitingMarker':''}"><span class="leaderRank">${r.tied?'T':''}${r.rank}</span><span class="leaderName">${esc(r.name)}${r.detail?`<small>${esc(r.detail)}</small>`:''}</span><span class="leaderScore">${waiting?'—':`${r.total} <small>${unit}</small>${r.cb?' <small>CB</small>':''}`}</span><span class="leaderThru">${waiting?'Waiting for Marker to Score':r.thru===r.target?'Final':`${r.target-r.thru} pending`}</span></div>`}).join('')||'<div class="leaderEmpty">No players are available.</div>'}
 host.innerHTML=`<div class="leaderHead"><div><h2>Live Leaderboard</h2><p>${esc(store.event.name)} · official marker scores update as they arrive</p></div><button class="soft leaderRefresh" id="leaderRefresh">Refresh</button></div>${viewTabs}${tabs}<div class="leaderCard"><div class="leaderTitle"><h3>${esc(def.label)}</h3><span>${def.countback?'Automatic countback':'Live standings'}</span></div>${body}</div><p class="leaderNote">Pending holes have not yet been entered by the player’s marker. They are never counted as zero. Finalisation also requires the player’s checking scores to agree. CB means countback.</p>`;
