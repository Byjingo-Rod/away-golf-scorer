...
function calculateLeaderboard(def){
 ...
 if(def.type==='scratch'){
  const v=version(course(def.day===1?store.event.course1:store.event.course2))||{};
  rows=field(def.day).map(id=>{const scores=gross(def.day,id),r=leaderRow(id,player(id)?.name||'Player','',scores);r.grossTotal=r.total;r.total=scores.reduce((n,g,i)=>n+(g==null?0:g-(+v.par?.[i]||0)),0);r.scratchToPar=r.total;return r});
 }
